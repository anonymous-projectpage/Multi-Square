...
def _strip_goal_item(goal_line: str) -> str:
    ...
    g = re.sub('^Goal:\\s*', '', g, flags=re.IGNORECASE).strip()
    g = g.rstrip('.').strip()
    g = re.sub('^craft\\s+', '', g, flags=re.IGNORECASE).strip()
    return g
def parse_textcraft_initial(obs0: str) -> Tuple[str, str]:
    text = (obs0 or '').strip()
    text = re.sub('^\\s*Instruction:\\s*\\n', '', text, flags=re.IGNORECASE)
    goal_line = ''
    m_goal = re.search('(?im)^\\s*Goal:\\s*(.+?)\\s*$', text)
    if m_goal:
        goal_line = m_goal.group(1).strip().rstrip('.').strip()
    commands_block = ''
    m_cmd = re.search('(?is)(Crafting commands:\\s*\\n.*?)(?:\\n\\s*Goal:\\s*|$)', text)
    if m_cmd:
...
def _normalize_ws(s: str) -> str:
    return re.sub('\\s+', ' ', (s or '').strip()).strip()
...
def parse_relevant_indices(raw: str) -> List[int]:
    if not raw:
        return []
    m = re.search('indices\\s*:\\s*\\[([0-9,\\s]+)\\]', raw, flags=re.IGNORECASE)
    if not m:
        return []
    nums: List[int] = []
...
def _parse_craft_line(line: str) -> Tuple[Optional[str], List[str]]:
    m = re.match('^craft\\s+\\d+\\s+(.+?)\\s+using\\s+(.+)$', (line or '').strip(), flags=re.IGNORECASE)
    if not m:
        return (None, [])
    ...
    inputs: List[str] = []
    for x in inputs_raw:
        x = re.sub('\\(need craft\\)', '', x, flags=re.IGNORECASE).strip()
        x = _strip_qty_item(x)
...
def _extract_needed_items_from_subtask(subtask: str) -> List[str]:
    s = (subtask or '').strip()
    m = re.search('\\(may need:\\s*(.+?)\\)\\s*for\\s+', s, flags=re.IGNORECASE)
    if not m:
        return []
    inside = m.group(1)
    ...
    out: List[str] = []
    for p in parts:
        p = re.sub('\\(need craft\\)', '', p, flags=re.IGNORECASE).strip()
        p = _strip_qty_item(p)
        if p:
            out.append(p)
    return out
...
def build_relevant_commands_block_auto(commands_block: str, *, rel_indices: List[int], subtask: str, goal_line: str) -> str:
    ...
    if sub_l.startswith('acquire needed items'):
        ...
        mm = re.search('craft target\\s*:\\s*(.+)$', sub, flags=re.IGNORECASE)
        if mm:
...
def sanitize_textcraft_action(action: str, allowed_crafts: Optional[set]=None) -> str:
    raw = action or ''
    s = raw.strip()
    s = re.sub('^\\s*(Action|action)\\s*:\\s*', '', s)
    s = re.sub('^\\s*>\\s*', '', s)
    lines = [ln.strip() for ln in s.splitlines() if ln.strip()]
    s = lines[0] if lines else ''
    if not s:
        parts = [p.strip() for p in re.split('[;|]', raw) if p.strip()]
    ...
    if not s:
        return 'inventory'
    s = re.sub('(?i)(?<!^)(?<!\\s)(inventory|get|craft)\\b', ' \\1', s)
    s_low = s.lower()
    if s_low.startswith('think'):
        return 'inventory'
    m_syn = re.match('^(find|gather|acquire|pickup|pick up)\\s+(.+)$', s_low)
    if m_syn:
        s = 'get ' + m_syn.group(2).strip()
        s_low = s.lower()
    if s_low in ('inv', 'inventory'):
        return 'inventory'
    m = re.search('\\b(inventory|get|craft)\\b', s_low)
    if m:
        s = s[m.start():].strip()
        s_low = s.lower()
    if s_low == 'inventory':
        ...
        if allowed_crafts:
            ...
            for ac in allowed_crafts:
                ...
                if norm in ac_norm or ac_norm in norm:
                    return ac
            m_use = re.search('\\busing\\b\\s+(.+)$', s, flags=re.IGNORECASE)
            if m_use:
...
class EvalAgent:
    def __init__(self, args: Dict[str, Any]):
        ...
        high_path = args.get('high_path') or f"{args['check_path']}/{args['benchmark']}/multi_bc/Qwen/Qwen2.5-3B-Instruct/0.0001/detail3/2026-01-27_20-28/best"
        low_path = args.get('low_path') or f"{args['check_path']}/{args['benchmark']}/multi_rl/detail3/Qwen/Qwen2.5-3B-Instruct/A0.0001_C1e-05/lamb7_beta10/low/2026-01-27_20-14/20000"
        ...
        if not os.path.isdir(recipes_dir):
            raise FileNotFoundError(f"TextCraft recipes dir not found: {recipes_dir}. Set args['textcraft_dir'] to the folder containing recipes/.")
        ...
        self.log_file = args.get('eval_log_file', os.path.join(base_dir, f'proposed22_Qwen_eval_results_20000_{run_ts}_nohigh.tsv'))
        self.tok_out = args.get('token_log_file', os.path.join(base_dir, 'token_stats', f'proposed22_Qwen_eval_results_20000_{run_ts}_nohigh.tsv'))
        ...
        if not os.path.exists(self.tok_out):
            with open(self.tok_out, 'w', encoding='utf-8') as f:
                f.write('split\tseed\tlabel\tscore\twon\tsteps\thigh_calls\thigh_in_sum\thigh_out_sum\thigh_ctx_max\tlow_calls\tlow_in_sum\tlow_out_sum\tlow_ctx_max\ttotal_tokens\ttoken_eff\n')
    ...
    def _append_token_log(self, split: str, seed: int, label: str, score: float, won: bool, steps: int, tok: Dict[str, float]):
        ...
        line = f"{split}\t{seed}\t{label}\t{score:.4f}\t{int(won)}\t{steps}\t{tok['high_calls']}\t{tok['high_in_sum']}\t{tok['high_out_sum']}\t{tok['high_ctx_max']}\t{tok['low_calls']}\t{tok['low_in_sum']}\t{tok['low_out_sum']}\t{tok['low_ctx_max']}\t{total_tokens}\t{token_eff:.8f}\n"
    ...
    def print_step_log(self, hi_turn: int, lo_turn: int, step: int, obs: str, raw_action: str, parsed_action: str, subtask_done: bool, r: float, done: bool):
    ...
    def eval_policy(self, seed: int, label: str='Unknown', split_label: str='dev') -> Tuple[float, Dict[str, float], int, str]:
        ...
        for hi_turn in range(self.max_high_turns):
            ...
            rel_block = build_relevant_commands_block_auto(commands_block, rel_indices=[], subtask=subtask, goal_line=goal_line)
            if rel_block:
            ...
            for lo_turn in range(self.max_low_turns):
                ...
                m = re.match('^(.*?);\\s*(true|false)\\s*$', raw_action, flags=re.IGNORECASE)
                if m:
                ...
                if self.debug:
                    self.print_step_log(hi_turn=hi_turn, lo_turn=lo_turn, step=episode_steps, obs=obs, raw_action=raw_action, parsed_action=parsed_action, subtask_done=bool(subtask_done), r=float(r), done=bool(done))
    ...
    def evaluate_split_env_variations(self, split: str='dev', annotation_path: Optional[str]=None, task_filter=None, max_episodes: Optional[int]=None, seed_start: int=0, seed_list: Optional[List[int]]=None) -> float:
